...
export default function SuperAdminLoginPage() {
  ...
  const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok || !payload.success || !payload.admin) {
        setError(payload.error ?? "Invalid credentials for Super Admin portal.");
        return;
      }

      setSuccess(true);
      setTimeout(() => router.push("/admin/super-admin/dashboard"), 800);
    } catch {
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-slate-950 via-violet-950/20 to-slate-950 flex flex-col justify-between py-10 px-4">
      {/* Top Section */}
      ...
      <div className="w-full max-w-md mx-auto my-auto">
        ...
        <Card theme="dark" noPad className="p-8 border border-amber-500/20 bg-slate-900/60 backdrop-blur-md shadow-2xl">
          {/* Header */}
          <div className="flex flex-col items-center gap-3 mb-6">
            <div className="flex items-center justify-center h-14 w-14 rounded-2xl bg-amber-500/10 border border-amber-500/20 text-amber-400 shadow-lg shadow-amber-500/5">
              <Crown className="h-7 w-7" />
            </div>
            <div className="text-center">
              <h1 className="text-2xl font-extrabold text-white tracking-tight">Super Admin Portal</h1>
              <p className="text-sm text-slate-400 mt-1">
                Sign in to your dedicated server settings dashboard
              </p>
            </div>
          </div>
          ...
          <form className="space-y-5" onSubmit={onSubmit}>
            ...
            <Button
              ...
              className="mt-1 bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-500 hover:to-orange-500 border-none"
            >
              Sign In to Super Portal
            </Button>
          </form>
          ...
          <div className="mt-6 flex flex-col items-center gap-3">
            ...
            <p className="text-sm text-slate-400">
              Need server setup?{" "}
              <Link
                href="/admin/super-admin/register"
