  ...
  const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch("/api/auth/admin/signin", {
        ...
        body: JSON.stringify({ email, password, role: "department_admin" }),
      });
      ...
      if (!response.ok || !payload.success || !payload.admin) {
        setError(payload.error ?? "Invalid credentials for Department Admin portal.");
        return;
      }

      setSuccess(true);
      setTimeout(() => router.push("/admin/department-admin/dashboard"), 800);
    } catch {
        ...
        <Card theme="dark" noPad className="p-8 border border-emerald-500/20 bg-slate-900/60 backdrop-blur-md shadow-2xl">
          ...
          <div className="flex flex-col items-center gap-3 mb-6">
            ...
            <div className="text-center">
              <h1 className="text-2xl font-extrabold text-white tracking-tight">Department Admin</h1>
              <p className="text-sm text-slate-400 mt-1">
                Sign in to your dedicated department portal
              </p>
            </div>
          </div>
          ...
          <form className="space-y-5" onSubmit={onSubmit}>
            ...
            <Button
              ...
              className="mt-1 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 border-none"
            >
