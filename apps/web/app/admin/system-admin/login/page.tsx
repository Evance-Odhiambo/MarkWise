...
export default function SystemAdminLoginPage() {
  ...
  const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch("/api/auth/admin/signin", {
        ...
        body: JSON.stringify({ email, password, role: "system_admin" }),
      });
      ...
      if (!response.ok || !payload.success || !payload.admin) {
        setError(payload.error ?? "Invalid credentials for System Administrator portal.");
        return;
      }

      setSuccess(true);
      setTimeout(() => router.push("/admin/system-admin/dashboard"), 800);
    } catch {
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-slate-950 via-indigo-950 to-slate-950 flex flex-col justify-between py-10 px-4">
      {/* Top Section */}
      ...
      <div className="w-full max-w-md mx-auto my-auto">
        ...
        <Card theme="dark" noPad className="p-8 border border-indigo-500/20 bg-slate-900/60 backdrop-blur-md shadow-2xl">
          {/* Header */}
          <div className="flex flex-col items-center gap-3 mb-6">
            <div className="flex items-center justify-center h-14 w-14 rounded-2xl bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 shadow-lg shadow-indigo-500/5">
              <Building2 className="h-7 w-7" />
            </div>
            <div className="text-center">
              <h1 className="text-2xl font-extrabold text-white tracking-tight">System Admin</h1>
              <p className="text-sm text-slate-400 mt-1">
                Sign in to your dedicated administration portal
              </p>
            </div>
          </div>
          ...
          <form className="space-y-5" onSubmit={onSubmit}>
            ...
            <Button
              ...
              className="mt-1 bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 border-none"
            >
