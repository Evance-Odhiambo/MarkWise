...
export default function AcademicRegistrarLoginPage() {
  ...
  const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch("/api/auth/admin/signin", {
        ...
        body: JSON.stringify({ email, password, role: "academic_registrar" }),
      });
      ...
      if (!response.ok || !payload.success || !payload.admin) {
        setError(payload.error ?? "Invalid credentials for Academic Registrar portal.");
        return;
      }

      setSuccess(true);
      setTimeout(() => router.push("/admin/academic-registrar/dashboard"), 800);
    } catch {
        ...
        <Card theme="dark" noPad className="p-8 border border-violet-500/20 bg-slate-900/60 backdrop-blur-md shadow-2xl">
          ...
          <div className="flex flex-col items-center gap-3 mb-6">
            ...
            <div className="text-center">
              <h1 className="text-2xl font-extrabold text-white tracking-tight">Academic Registrar</h1>
              <p className="text-sm text-slate-400 mt-1">
                Sign in to your dedicated registrar portal
              </p>
            </div>
          </div>
          ...
          <form className="space-y-5" onSubmit={onSubmit}>
            ...
            <Button
              ...
              className="mt-1 bg-gradient-to-r from-violet-600 to-fuchsia-600 hover:from-violet-500 hover:to-fuchsia-500 border-none"
            >
              Sign In to Registrar Portal
            </Button>
          </form>
